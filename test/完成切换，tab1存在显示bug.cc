...
public:
    ...
    void SetupTab1() {
        lv_obj_t* fake_screen = lv_obj_create(tab1_);
        lv_obj_set_size(fake_screen, LV_HOR_RES, LV_VER_RES);
        lv_obj_clear_flag(fake_screen, LV_OBJ_FLAG_SCROLLABLE);
        auto screen = fake_screen;
        ...
        lv_obj_set_style_bg_color(container, current_theme_.background, 0);
        lv_obj_set_style_border_color(container, current_theme_.border, 0);

        // Status bar
        lv_obj_t* status_bar = lv_obj_create(container);
        lv_obj_set_size(status_bar, LV_HOR_RES, LV_SIZE_CONTENT);
        lv_obj_set_style_radius(status_bar, 0, 0);
        lv_obj_set_style_bg_color(status_bar, current_theme_.background, 0);
        lv_obj_set_style_text_color(status_bar, current_theme_.text, 0);
        ...
        // emotion_label 在状态栏最左侧
        lv_obj_t* emotion_label = lv_label_create(status_bar);
        lv_obj_set_style_text_font(emotion_label, &font_awesome_30_4, 0);
        lv_obj_set_style_text_color(emotion_label, current_theme_.text, 0);
        lv_label_set_text(emotion_label, FONT_AWESOME_AI_CHIP);
        lv_obj_set_style_margin_right(emotion_label, 5, 0);

        // notification_label
        lv_obj_t* notification_label = lv_label_create(status_bar);
        lv_obj_set_flex_grow(notification_label, 1);
        lv_obj_set_style_text_align(notification_label, LV_TEXT_ALIGN_CENTER, 0);
        lv_obj_set_style_text_color(notification_label, current_theme_.text, 0);
        ...
        lv_obj_set_style_text_color(status_label, current_theme_.text, 0);
        lv_label_set_text(status_label, Lang::Strings::INITIALIZING);

        // mute_label
        lv_obj_t* mute_label = lv_label_create(status_bar);
        lv_label_set_text(mute_label, "");
        lv_obj_set_style_text_font(mute_label, fonts_.icon_font, 0);
        lv_obj_set_style_text_color(mute_label, current_theme_.text, 0);

        // network_label
        lv_obj_t* network_label = lv_label_create(status_bar);
        lv_label_set_text(network_label, "");
        lv_obj_set_style_text_font(network_label, fonts_.icon_font, 0);
        lv_obj_set_style_text_color(network_label, current_theme_.text, 0);
        lv_obj_set_style_margin_left(network_label, 5, 0);

        // battery_label
        lv_obj_t* battery_label = lv_label_create(status_bar);
        lv_label_set_text(battery_label, "");
        lv_obj_set_style_text_font(battery_label, fonts_.icon_font, 0);
        lv_obj_set_style_text_color(battery_label, current_theme_.text, 0);
        ...
        lv_obj_set_style_bg_color(content, current_theme_.chat_background, 0);
        lv_obj_set_style_border_color(content, current_theme_.border, 0);
        ...
        // 低电量弹窗
        lv_obj_t* low_battery_popup = lv_obj_create(screen);
        lv_obj_set_scrollbar_mode(low_battery_popup, LV_SCROLLBAR_MODE_OFF);
        lv_obj_set_size(low_battery_popup, LV_HOR_RES * 0.9, fonts_.text_font->line_height * 2);
        lv_obj_align(low_battery_popup, LV_ALIGN_BOTTOM_MID, 0, 0);
        lv_obj_set_style_bg_color(low_battery_popup, current_theme_.low_battery, 0);
        lv_obj_set_style_radius(low_battery_popup, 10, 0);
        lv_obj_t* low_battery_label = lv_label_create(low_battery_popup);
        ...
        // idle计时器
        SetIdle(true);
        lv_obj_add_event_cb(fake_screen, [](lv_event_t* e) {
            if (lv_event_get_code(e) == LV_EVENT_CLICKED) {
                auto* display = static_cast<CustomLcdDisplay*>(lv_event_get_user_data(e));
    ...
    void SetIdle(bool status) {
        ...
        idle_timer_ = lv_timer_create([](lv_timer_t* t) {
            ...
            if (!display) return;
            lv_tabview_set_act(display->tabview_, 1, LV_ANIM_OFF);
            lv_timer_del(t);
